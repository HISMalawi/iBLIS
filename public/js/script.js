 ...
 **/
	...
	function reportShowSpinerDisableBtn(){
		showSpinner(null, false, false);
		document.getElementById("filter").disabled = true; 
		document.getElementById("btnExport").disabled = true;
	}

	function reportHideSpinerEnableBtn(){
...
async function mohReportProcessData(year, indicators, counter, quarters, department) {
  for (let i = 0; i < indicators.length; i++) {
    ...
    for (let q = 0; q < quarters.length; q++) {
      ...
      const url = `/moh_diagnostic_stats?indicator=${indicators[i]}&department=${department}&year=${year}&quarter=${quarter}`;
      const slotCounter = q % 4;
      try{
				const res = await $.ajax({ url, async: true });
				const [month1, month2, month3] = res.slice(0, 3).map(x => x[1]);
          const id1 = counter + "-" + (slotCounter * 4 + 1);
          const id2 = counter + "-" + (slotCounter * 4 + 2);
          const id3 = counter + "-" + (slotCounter * 4 + 3);
          const id4 = counter + "-" + (slotCounter * 4 + 4);
          document.getElementById(id1).innerHTML = month1;
          document.getElementById(id1).style.color = "green";
          document.getElementById(id2).innerHTML = month2;
          document.getElementById(id2).style.color = "green";
          document.getElementById(id3).innerHTML = month3;
          document.getElementById(id3).style.color = "green";
          document.getElementById(id4).innerHTML = parseInt(month1) + parseInt(month2) + parseInt(month3);
          document.getElementById(id4).style.color = "white";
          total += parseInt(month1) + parseInt(month2) + parseInt(month3);
          if (slotCounter === 3) {
            const id5 = counter + "-17";
            document.getElementById(id5).innerHTML = total;
