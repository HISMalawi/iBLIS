...
class Specimen extends Eloquent
{
    ...
    */
	...
	public static function assignAccessionNumber(){
		...
		if (Config::get('app.useDefaultAccessionNumberLength')) {
			$sentinel = 99999999;
			$total_length = 8;
		}
		else {
		...
		if(COUNT($record) > 0){
			$max_acc_num = (int)substr($record[0]->accession_number, -$total_length);
		...
		$max_acc_num = str_pad($max_acc_num, $total_length, '0', STR_PAD_LEFT);
		$year = date('y');
